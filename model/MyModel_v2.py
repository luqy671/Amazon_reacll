...
class MyModel_v2(Multi_Intere_Model):
    
    def __init__(self, config):
        super(MyModel_v2, self).__init__(config)
    ...
    def Intere_build(self, seq_embed):  #[batch, seq_l, embed_dim]
        ...
        '''
        norm_dim = intere_vec.ndim -1
        norm2 = torch.norm(intere_vec, dim=norm_dim).unsqueeze(dim=norm_dim)
        #norm2 += 1e9
        norm_intere_vec = intere_vec/norm2
        '''
        return intere_vec
