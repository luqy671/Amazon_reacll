...
class MyModel_v3(Multi_Intere_Model):
    ...
    def __init__(self, config):
        ...
        if self.classifier > 0:
            ...
            self.classifier_layer = nn.Sequential(
                ...
                nn.Linear(self.embed_dim, self.Candidate_num))
    ...
    def Candidate_classifier(self, seq_embed):#[batch, seq_l, embed_dim]
        MLP_input = torch.mean(seq_embed,dim=1) #[batch,embed_dim]
        class_score = self.classifier_layer(MLP_input) #[batch,Candidate_num]
        
        # 对每个batch内的得分，降序排列
        sorted_score, indices = torch.sort(class_score, descending = True)  #[batch, Candidate_num]
        # 取 top k
        sorted_scor_K, indices_K = sorted_score[:, : self.intere_num], indices[:, : self.intere_num]  #[batch, intere_num]
        activated_Candidate = self.Candidate_pool[indices_K] #[batch, intere_num, embed_dim]
        
        # 计算 top k 个兴趣的 attention_score
        Candidate_att_score = F.softmax(torch.einsum('ijk, ilk -> ijl', seq_embed, activated_Candidate), dim=1) #[batch, seq_l, intere_num]
        return Candidate_att_score, indices_K
    
    def Intere_build(self, seq_embed):  #[batch, seq_l, embed_dim]
        if self.classifier > 0:
            Candidate_att, indices_K = self.Candidate_classifier(seq_embed) #[batch, seq_l, intere_num] [batch, intere_num]
        else:
